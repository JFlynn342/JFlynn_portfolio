...
namespace Animation {
	/// <summary>
	/// A Transition-derived class that defines a transition that occurs over time
	/// </summary>
	class BlendTransition : public Transition {
	protected:
		BlendTransition(Library::AnimationPlayer* player, Library::Game& game, const std::shared_ptr<Library::AnimationClip>& from, const std::shared_ptr<Library::AnimationClip>& to, float duration, uint32_t keyframe);
		/// <summary>
		...
		/// <summary>
		/// maps each bone to its origin and target transforms
		/// </summary>
		...
		virtual void GetBlendedTransform(Library::Bone& bone, DirectX::XMFLOAT4X4& transform) = 0;
		/// <summary>
